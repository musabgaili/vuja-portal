    ...
    @foreach([
        ...
        <div class="col-md-3">
            <div class="card"><div class="card-content">
                ...
                <div style="font-size:1.75rem; font-weight:800; color:var(--primary-color);">{{ $c['money'] ? '$'.number_format($c['val'],0) : $c['val'] }}</div>
            </div></div>
        </div>
    @endforeach
</div>
    ...
    @foreach($columns as $key => $col)
        <div class="crm-col" data-stage="{{ $key }}">
            <div class="crm-col-head">
                ...
                <span class="text-muted" style="font-size:.8rem;">${{ number_format($col['value'], 0) }}</span>
            </div>
                ...
                @foreach($col['items'] as $opp)
                    <div class="crm-card" draggable="true" data-id="{{ $opp->id }}">
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-2">
                            <span style="font-weight:700; color:var(--primary-color);">${{ number_format((float)$opp->expected_value, 0) }}</span>
